...
WHERE MEMBER_ID IN (SELECT MEMBER_ID
					FROM MEMBER
					WHERE AGE BETWEEN 30 AND 39 AND SEX = 'f' );
...
DELETE FROM CAR_BAK WHERE CAR_CD NOT IN (SELECT CAR_CD FROM RENT);
...
#SELECT 	PRICE - AVG(PRICE) FROM 	CAR   WHERE 	CAR_NM = 'k7'; 오답!
...
FROM (SELECT * 
	  FROM CAR
      WHERE PRICE > (SELECT AVG(PRICE) FROM CAR)) T1;
...
FROM ( SELECT *
		FROM  CAR
		WHERE CAR_CD IN (SELECT CAR_CD FROM RENT WHERE RENT_PERIOD >= 5)) T1;
...
FROM ( SELECT *
	   FROM	  RENT
       WHERE  CAR_CD IN (SELECT CAR_CD FROM CAR WHERE BRAND_NM = '기아' )) AS T1;
